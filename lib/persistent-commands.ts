...
export async function createPersistentCommand(id:string,deviceId:string|number,command:string,payload:unknown){
  if(!databaseConfigured())return null; try{
    const r=await query("INSERT INTO device_commands (id, device_id, command, payload, status) VALUES ($1,$2,$3,$4::jsonb,'queued') ON CONFLICT (id) DO NOTHING RETURNING id,device_id,command,payload,status,created_at,sent_at,acked_at,result",[id,String(deviceId),command,JSON.stringify(payload??null)]);
    return r.rows[0]?normalize(r.rows[0] as Record<string,unknown>):null;
  }catch{return null;}
}
export async function markPersistentCommandSent(id:string){
  if(!databaseConfigured())return null; try{
    const r=await query("UPDATE device_commands SET status='sent', sent_at=COALESCE(sent_at,now()) WHERE id=$1 AND status='queued' RETURNING id,device_id,command,payload,status,created_at,sent_at,acked_at,result",[id]);
...
export async function requeuePersistentCommand(id:string){
  if(!databaseConfigured())return null; try{
    const r=await query("UPDATE device_commands SET status='queued', sent_at=NULL, acked_at=NULL, result=NULL WHERE id=$1 AND status='sent' RETURNING id,device_id,command,payload,status,created_at,sent_at,acked_at,result",[id]);
...
export async function getPersistentCommand(id:string){
  if(!databaseConfigured())return null; try{const r=await query("SELECT id,device_id,command,payload,status,created_at,sent_at,acked_at,result FROM device_commands WHERE id=$1 LIMIT 1",[id]);return r.rows[0]?normalize(r.rows[0] as Record<string,unknown>):null;}catch{return null;}
}
export async function listPersistentPendingCommands(deviceId:string|number,limit=10){
  if(!databaseConfigured())return []; try{const r=await query("SELECT id,device_id,command,payload,status,created_at,sent_at,acked_at,result FROM device_commands WHERE device_id=$1 AND status='queued' ORDER BY created_at ASC LIMIT $2",[String(deviceId),safeLimit(limit)]);return r.rows.map(x=>normalize(x as Record<string,unknown>));}catch{return [];}
}
...
export async function claimPersistentCommands(deviceId:string|number,limit=10,recoveryCommandId?:string){
  if(!databaseConfigured())return [];try{
    ...
    const r=await query("WITH picked AS (SELECT id FROM device_commands WHERE device_id=$1 AND ((status='queued') OR (status='sent' AND sent_at IS NOT NULL AND id=$3)) ORDER BY CASE WHEN id=$3 THEN 0 ELSE 1 END, created_at ASC LIMIT $2 FOR UPDATE SKIP LOCKED) UPDATE device_commands c SET status='sent', sent_at=COALESCE(c.sent_at,now()) FROM picked WHERE c.id=picked.id RETURNING c.id,c.device_id,c.command,c.payload,c.status,c.created_at,c.sent_at,c.acked_at,c.result",[String(deviceId),safeLimit(limit),normalizedRecovery]);
