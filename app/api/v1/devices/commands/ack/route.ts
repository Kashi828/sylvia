...
export async function POST(request:Request){
 ...
 const item=ackCommand(device.id,body.commandId,body.result??null);if(!item)return NextResponse.json({ok:false,error:'Command not found or already acknowledged'},{status:404});
 device.online=true;device.lastSeen=new Date().toISOString();addEvent('device.command.ack',`${device.name}: ${item.command} acknowledged`,device.id);
