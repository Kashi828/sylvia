...
export async function POST(request:Request,{params}:{params:Promise<{id:string}>}){
  const limited=withRateLimit(request,30);
  ...
  const persistentByPrincipal=auth ? await findPersistentDeviceById(id,auth.ownerId,auth.projectId) : null;
  const numericId=Number(id);
  const legacyDevice=Number.isFinite(numericId) ? findDevice(numericId) : null;

  const device=persistentByToken || persistentByPrincipal || (
    !auth && rawToken && legacyDevice && validBearer(rawToken,numericId) ? legacyDevice : null
  );
  if(!device)return NextResponse.json({ok:false,error:"Unauthorized or device not found"},{status:401});

  if(auth && !persistentByPrincipal && !persistentByToken){
    return NextResponse.json({ok:false,error:"Device not found in the selected project"},{status:404});
  }
  ...
  const persistentCommand=persistentReady ? await createPersistentCommand(commandId,device.id,command,payload) : null;
  ...
  if(persistentReady && !persistentCommand){
    ...
    return NextResponse.json({ok:false,error:"Persistent command storage unavailable; command was not dispatched",commandId},{status:503});
  }
  ...
  if(mqttStatus().configured){
    try{
      ...
      await recordDeviceEvent({
        ...
        kind:"device.command.dispatched",
        severity:"success",
        message:device.name+": "+command+" dispatched",
        data:{commandId,command,transport:"mqtt"},
      });
    }catch(error){
      ...
      await recordDeviceEvent({
        ...
        ownerId:auth?.ownerId,
        projectId:auth?.projectId,
        kind:"device.command.dispatch_failed",
        severity:"warning",
        message:device.name+": "+command+" remained queued",
        data:{commandId,command,error:dispatchError||"unknown"},
      });
    }
  }else{
  ...
  await recordAuditEvent({
    ...
    actorType:auth?.method || "device",
    actorId:auth?.userId || String(device.id),
    action:"device_command.dispatched",
    resourceType:"device",
    resourceId:String(device.id),
    metadata:{command,commandId,transport:dispatched?"mqtt":"queue",dispatched,projectId:auth?.projectId},
    request,
  });
  ...
  return NextResponse.json({
    ...
    dispatched,
    dispatchError,
    transport:dispatched?"mqtt":"queue",
    command,
    payload,
    commandId,
    projectId:auth?.projectId,
