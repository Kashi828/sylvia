...
export async function POST(req:Request){
  ...
  const user=verifyCredentials(String(body.email||''),String(body.password||''));
  if(!user)return NextResponse.json({ok:false,error:'Invalid email or password'},{status:401});
  const token=createSession(user.id); const res=NextResponse.json({ok:true,user:publicUser(user)});
