...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (stream.type === 'Number') {
    ...
    if (!Number.isFinite(n)) return NextResponse.json({ ok: false, error: 'Value must be numeric' }, { status: 400 });
    value = n;
  }
  if (stream.type === 'Boolean') {
    if (typeof value === 'string') value = value === 'true';
    if (typeof value !== 'boolean') return NextResponse.json({ ok: false, error: 'Value must be boolean' }, { status: 400 });
  }
  ...
  addEvent('device.state', `${device.name} state → ${stream.name}: ${String(value)}`, device.id, stream.id);
