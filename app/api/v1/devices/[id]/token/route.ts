...
export async function POST(request:Request,{params}:{params:Promise<{id:string}>}){
  ...
  if(auth&&auth.method==="session"){
    ...
    if(action==="revoke"){
      ...
      await recordAuditEvent({ownerId:auth.ownerId,actorType:"session",actorId:auth.userId!,action:"device_token.revoked",resourceType:"device",resourceId:id,metadata:{generation:device.tokenGeneration??1,revoked:changed},request});
    ...
    await recordAuditEvent({ownerId:auth.ownerId,actorType:"session",actorId:auth.userId!,action:"device_token.rotated",resourceType:"device",resourceId:id,metadata:{generation:rotated.device.tokenGeneration??1},request});
