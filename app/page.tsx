...
function AutomationPanel({rules,streams,devices,onAdd,onDelete,onToggle,onTest}:{rules:Rule[];streams:Stream[];devices:Device[];onAdd:()=>void;onDelete:(id:number)=>void;onToggle:(id:number)=>void;onTest:(r:Rule)=>void}){
 const [cloudRules,setCloudRules]=useState<any[]>([]); const [runs,setRuns]=useState<any[]>([]); const [open,setOpen]=useState(false); const [loading,setLoading]=useState(false); const [message,setMessage]=useState('');
 const [draft,setDraft]=useState({name:'',streamId:0,operator:'>',threshold:'30',action:'event'});
 const load=async()=>{setLoading(true);try{const [a,b]=await Promise.all([fetch('/api/v1/automations',{cache:'no-store'}),fetch('/api/v1/automation-runs?limit=8',{cache:'no-store'})]);const aj=await a.json().catch(()=>null);const bj=await b.json().catch(()=>null);if(a.ok)setCloudRules(Array.isArray(aj?.automations)?aj.automations:[]);if(b.ok)setRuns(Array.isArray(bj?.runs)?bj.runs:[]);}catch{setMessage('Automation API unavailable')}finally{setLoading(false)}};
 ...
 const localStream=(remoteId:string)=>streams.find(s=>String(s.remoteId)===String(remoteId));
 const create=async()=>{const stream=streams.find(s=>s.id===draft.streamId);if(!draft.name.trim()||!stream?.remoteId){setMessage('Choose a persisted datastream and automation name');return;}try{const action=draft.action==='device_command'?'device_command':'event';const r=await fetch('/api/v1/automations',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({name:draft.name.trim(),deviceId:String(stream.deviceId),streamId:String(stream.remoteId),operator:draft.operator,threshold:Number(draft.threshold),action,command:action==='device_command'?'digital_write':null,payload:action==='device_command'?{pin:2,value:1}:{},cooldownSeconds:300,enabled:true})});const j=await r.json().catch(()=>null);if(!r.ok)throw new Error(j?.error||'Automation creation failed');setOpen(false);setDraft({name:'',streamId:stream.id,operator:'>',threshold:'30',action:'event'});setMessage('Automation saved to PostgreSQL');load()}catch(error){setMessage(error instanceof Error?error.message:'Automation creation failed')}};
 const toggle=async(item:any)=>{try{const r=await fetch('/api/v1/automations',{method:'PATCH',headers:{'Content-Type':'application/json'},body:JSON.stringify({id:item.id,enabled:!item.enabled})});const j=await r.json().catch(()=>null);if(!r.ok)throw new Error(j?.error||'Automation update failed');load()}catch(error){setMessage(error instanceof Error?error.message:'Automation update failed')}};
 const remove=async(item:any)=>{try{const r=await fetch('/api/v1/automations?id='+encodeURIComponent(String(item.id)),{method:'DELETE'});const j=await r.json().catch(()=>null);if(!r.ok||j?.deleted===false)throw new Error(j?.error||'Automation deletion failed');load()}catch(error){setMessage(error instanceof Error?error.message:'Automation deletion failed')}};
 return <div className="apiPage">
  <div className="sectionHead"><div><span className="eyebrow">CLOUD AUTOMATION ENGINE · v0.56.0</span><h2>Telemetry rules that execute real device actions.</h2><span>Rules are stored in PostgreSQL, triggered by incoming telemetry, and linked to persistent command execution.</span></div><button className="primary" onClick={()=>setOpen(v=>!v)}><Plus size={15}/> {open?'Close':'Create automation'}</button></div>
  {message&&<div className="panel"><b>{message}</b></div>}
  {open&&<div className="panel"><div className="form"><label>Name<input autoFocus value={draft.name} onChange={e=>setDraft({...draft,name:e.target.value})} placeholder="Tank level control"/></label><div className="conditionRow"><label>Datastream<select value={draft.streamId} onChange={e=>setDraft({...draft,streamId:Number(e.target.value)})}>{streams.filter(s=>s.remoteId).map(s=><option key={s.id} value={s.id}>{devices.find(d=>d.id===s.deviceId)?.name||'Device'} · {s.name}</option>)}</select></label><label>Operator<select value={draft.operator} onChange={e=>setDraft({...draft,operator:e.target.value})}><option>&gt;</option><option>&lt;</option><option>=</option><option>!=</option></select></label><label>Threshold<input value={draft.threshold} onChange={e=>setDraft({...draft,threshold:e.target.value})}/></label></div><label>Action<select value={draft.action} onChange={e=>setDraft({...draft,action:e.target.value})}><option value="event">Create event</option><option value="device_command">Send digital_write</option></select></label><p className="modalHint">The first actuator path uses GPIO 2 / D2. Use a physical LED or isolated relay module for initial validation.</p><button className="primary full" onClick={create}>Save cloud automation</button></div></div>}
  <div className="panel"><div className="sectionHead"><div><h2>Active cloud rules</h2><span>{loading?'Refreshing…':cloudRules.length+' persistent automation(s)'}</span></div><span className="enabled">PostgreSQL</span></div><div className="ruleList">{cloudRules.map((item,index)=>{const s=localStream(item.streamId);return <div className="rule" key={String(item.id)}><div className="ruleNum">{String(index+1).padStart(2,'0')}</div><div className="ruleMain"><b>{item.name}</b><p>{s?(devices.find(d=>d.id===s.deviceId)?.name||'Device')+' · '+s.name:'Device '+item.deviceId+' · Stream '+item.streamId} {item.operator} {item.threshold} → {item.action}</p></div><span className={item.enabled?'enabled':'disabled'}>{item.enabled?'Enabled':'Paused'}</span><button className="mini" onClick={()=>toggle(item)}>{item.enabled?'Pause':'Enable'}</button><button className="trash" onClick={()=>remove(item)}><Trash2 size={15}/></button></div>})}{cloudRules.length===0&&<div className="emptyMini">No persistent automations configured.</div>}</div></div>
  <div className="panel"><div className="sectionHead"><div><h2>Execution history</h2><span>Every rule and schedule execution receives a durable run record.</span></div><span className="enabled">Durable</span></div>{runs.length?<div className="ruleList">{runs.map(run=><div className="rule" key={String(run.id)}><div className="ruleNum">{run.status==='dispatched'?'✓':run.status==='queued'?'Q':'!'}</div><div className="ruleMain"><b>{run.source_type==='schedule'?'Schedule':'Rule'} · {run.source_id}</b><p>Device {run.device_id} · {run.action}{run.command_id?' · '+run.command_id:''}{run.error?' · '+run.error:''}</p></div><span className={run.status==='failed'?'disabled':'enabled'}>{run.status}</span><span className="muted">{new Date(run.created_at).toLocaleString()}</span></div>)}</div>:<div className="emptyMini">No automation executions yet.</div>}</div>
 </div>
}
...
function SchedulesPanel({schedules,onToggle,onRun,onAdd,devices}:{schedules:Schedule[];onToggle:(id:number)=>void;onRun:(s:Schedule)=>void;onAdd:()=>void;devices:Device[]}){
 ...
 <div className="panel"><div className="apiTitle"><Bell size={17}/><div><b>Scheduler execution model</b><span>Every minute the worker checks each enabled schedule in its configured timezone, creates a persistent command, then records the execution result. Repeated cron calls are deduplicated.</span></div></div></div></div>
}
