...
function DashboardPanel({widgets,streams,onAdd,onDelete,onUpdate,setNoticeForDashboard}:{widgets:Widget[];streams:Stream[];onAdd:(k:Widget['kind'])=>void;onDelete:(id:number)=>void;onUpdate:(id:number,patch:Partial<Widget>)=>void;setNoticeForDashboard?:(v:string)=>void}){
 ...
 const sendSwitch=async(stream:Stream)=>{if(!stream.remoteId||stream.type!=='Boolean'){return}const next=String(stream.value)!=='true';setSwitchBusy(stream.id);try{const response=await fetch(`/api/v1/devices/${stream.deviceId}/command`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({command:'digital_write',payload:{pin:2,value:next?1:0,datastreamId:stream.remoteId}})});const data=await response.json().catch(()=>null);if(!response.ok)throw new Error(data?.error||'Command failed');setNoticeForDashboard?.(data?.dispatchState==='queued'?'Switch command queued':'Switch command sent')}catch(error){setNoticeForDashboard?.(error instanceof Error?error.message:'Command failed')}finally{setSwitchBusy(null)}};
 const [cloudHistory,setCloudHistory]=useState<Record<number,HistoryPoint[]>>({});
 useEffect(()=>{let active=true;const syncPhysicalState=async()=>{const targets=streams.filter(s=>s.type==='Boolean'&&s.remoteId);await Promise.all(targets.map(async s=>{try{const response=await fetch(`/api/v1/devices/${s.deviceId}`,{cache:'no-store'});if(!response.ok)return;const data=await response.json().catch(()=>null);const state=data?.state as Record<string,unknown>|undefined;if(!state)return;const key=String(s.name).toLowerCase().replace(/[^a-z0-9]/g,'');const relay=state.relayOn;const named=Object.entries(state).find(([k])=>k.toLowerCase().replace(/[^a-z0-9]/g,'')===key)?.[1];const value=typeof named==='boolean'?named:typeof relay==='boolean'?relay:undefined;if(typeof value==='boolean'&&active)setStreams(current=>current.map(item=>item.id===s.id?{...item,value}:item));}catch{}}))};void syncPhysicalState();const timer=setInterval(syncPhysicalState,3000);return()=>{active=false;clearInterval(timer)}},[streams.length]);
