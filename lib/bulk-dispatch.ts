...
export async function dispatchBulkCommand(
  ...
  for (const deviceId of group.deviceIds) {
    const result: DispatchResult = {
      deviceId,
      status: "queued",
      operationKey: `${id}:${deviceId}`,
    };

    if (!validBearer(token, deviceId)) {
      result.status = "rejected";
      result.error = "Unauthorized device token";
    ...
    try {
      ...
      result.error = error instanceof Error ? error.message : "Dispatch failed";
    }
...
export function getDispatchOperation(id: string) {
  return operations.get(id) ?? null;
}

export function listDispatchOperations() {
