...
export default function Home(){
 ...
 const initialProjectId=typeof window!=='undefined'?localStorage.getItem('sylvia.projectId')||'sylvia-local-workspace':'sylvia-local-workspace';
 const [projectId,setProjectId]=useState(initialProjectId); const [projects,setProjects]=useState<WorkspaceProject[]>([]); const [projectName,setProjectName]=useState('SYLVIA Cloud Project'); const [darkMode,setDarkMode]=useState(false); const [hydrated,setHydrated]=useState(false); const [tab,setTab]=useState('Overview'); const [notice,setNotice]=useState('');
 const [selectedDevice,setSelectedDevice]=useState<Device|null>(null); const [authUser,setAuthUser]=useState<{name:string;email:string;role:string}|null>(null); const [showReleaseNotes,setShowReleaseNotes]=useState(false); const [selectedStream,setSelectedStream]=useState<Stream|null>(null); const [history,setHistory]=useState<Record<number,HistoryPoint[]>>({});
 ...
 const switchProject=(id:string)=>{localStorage.setItem('sylvia.projectId',id);document.cookie=`sylvia_project=${encodeURIComponent(id)}; Path=/; Max-Age=31536000; SameSite=Lax`;setProjectId(id);window.location.reload()};
 const createProject=async()=>{const name=window.prompt('New project name');if(!name?.trim())return;try{const r=await fetch('/api/v1/projects',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({name:name.trim()})});const d=await r.json().catch(()=>null);if(!r.ok)throw new Error(d?.error||'Project creation failed');setProjects(p=>[...p,d.project]);setNotice('Project created');switchProject(d.project.id)}catch(error){setNotice(error instanceof Error?error.message:'Project creation failed')}};
 ...
 useEffect(()=>{
   const seenRelease=localStorage.getItem('sylvia.release.seen');
   if(seenRelease!==CURRENT_RELEASE){setShowReleaseNotes(true);localStorage.setItem('sylvia.release.seen',CURRENT_RELEASE)}
   fetch('/api/auth/me').then(r=>r.json()).then(d=>{if(d.authenticated){setAuthUser(d.user);setCurrentRole(d.user.role)}}).catch(()=>{});
   fetch('/api/v1/projects',{cache:'no-store'}).then(async response=>{if(!response.ok)return;const data=await response.json().catch(()=>null);const list=Array.isArray(data?.projects)?data.projects as WorkspaceProject[]:[];setProjects(list);const active=list.find(p=>p.id===projectId);if(active){setProjectName(active.name);setCurrentRole((active.role||'Viewer') as Member['role'])}}).catch(()=>{});
 ...
 return <main className={darkMode?'dark':''}><header><div className="accountStrip">{authUser ? <span>Signed in as <b>{authUser.name}</b> · {authUser.role}</span> : <span>Cloud workspace · PostgreSQL authentication available</span>}</div><div className="brand"><div className="logo">S</div><div><b>SYLVIA</b><span>IOT PLATFORM · v0.62.0 · REST + MQTT</span></div></div><div className="headerRight"><div className="status"><i/> Cloud workspace online</div><button className="releaseButton" onClick={()=>setShowReleaseNotes(true)} aria-label="Open release notes"><Bell size={14}/> What's new</button>{projects.length>0&&<select className="roleSelect" value={projectId} onChange={e=>switchProject(e.target.value)} aria-label="Active project">{projects.map(p=><option key={p.id} value={p.id}>{p.name}</option>)}</select>}<button className="themeToggle" onClick={()=>setDarkMode(v=>!v)} aria-label="Toggle theme">{darkMode?<Sun size={16}/>:<Moon size={16}/>}</button></div></header><div className="layout"><aside><nav>{nav.map(([label,Icon])=><button key={label} className={tab===label?'active':''} onClick={()=>setTab(label)}><Icon size={16}/><span>{label}</span></button>)}</nav><div className="sideNote"><b>{projectName}</b><small>{devices.length} devices · {online} online</small><button className="reset" onClick={resetWorkspace}><RotateCcw size={12}/> Clear workspace</button></div></aside><section className="content">
 ...
 {tab==='Workspace'&&<WorkspacePanel projectId={projectId} projects={projects} projectName={projectName} setProjectName={setProjectName} members={members} setMembers={setMembers} currentRole={currentRole} setCurrentRole={setCurrentRole} setNotice={setNotice} onCreateProject={createProject}/>}
 ...
 {showReleaseNotes&&<div className="overlay releaseOverlay" role="dialog" aria-modal="true" aria-labelledby="release-notes-title">
  <div className="releaseModal">
   <div className="releaseHeader">
    <div><span className="eyebrow">WHAT'S NEW · v{RELEASE_NOTES.version}</span><h2 id="release-notes-title">{RELEASE_NOTES.title}</h2><p>{RELEASE_NOTES.summary}</p></div>
    <button className="iconButton" onClick={()=>{localStorage.setItem('sylvia.release.seen',CURRENT_RELEASE);setShowReleaseNotes(false)}} aria-label="Close release notes"><X size={18}/></button>
   </div>
   <div className="releaseBody">
    <div className="releaseGrid">{RELEASE_NOTES.changes.map(section=><div className="releaseCard" key={section.title}><div className="releaseCardTitle"><span>✓</span><b>{section.title}</b></div><div className="releaseItems">{section.items.map(item=><div key={item}>✓ <span>{item}</span></div>)}</div></div>)}</div>
    <div className="releaseFooter"><span>Marked as seen for v{CURRENT_RELEASE}. The notice will return automatically with the next platform release.</span><button className="primary" onClick={()=>{localStorage.setItem('sylvia.release.seen',CURRENT_RELEASE);setShowReleaseNotes(false)}}>Got it</button></div>
   </div>
  </div>
 </div>
...
function WorkspacePanel({projectId,projects,projectName,setProjectName,members,setMembers,currentRole,setCurrentRole,setNotice,onCreateProject}:{projectId:string;projects:WorkspaceProject[];projectName:string;setProjectName:(v:string)=>void;members:Member[];setMembers:(v:Member[])=>void;currentRole:Member['role'];setCurrentRole:(v:Member['role'])=>void;setNotice:(v:string)=>void;onCreateProject:()=>void}){
...
function AutomationPanel({rules,streams,devices,onAdd,onDelete,onToggle,onTest}:{rules:Rule[];streams:Stream[];devices:Device[];onAdd:()=>void;onDelete:(id:number)=>void;onToggle:(id:number)=>void;onTest:(r:Rule)=>void}){
 ...
 return <div className="apiPage"><div className="sectionHead"><div><span className="eyebrow">CLOUD AUTOMATION ENGINE · v0.62.0</span><h2>Telemetry rules that execute real device actions.</h2><span>Rules are persisted in PostgreSQL and linked to persistent command execution.</span></div><button className="primary" onClick={()=>setOpen(v=>!v)}><Plus size={15}/>{open?"Close":"Create automation"}</button></div>
...
function SchedulesPanel({schedules,onToggle,onRun,onAdd,devices}:{schedules:Schedule[];onToggle:(id:number)=>void;onRun:(s:Schedule)=>void;onAdd:()=>void;devices:Device[]}){
 ...
 return <div className="apiPage"><div className="sectionHead"><div><span className="eyebrow">CLOUD SCHEDULER · v0.62.0</span><h2>Persistent time-based device commands.</h2><span>Schedules are stored in PostgreSQL and evaluated by the secured automation worker.</span></div><button className="primary" onClick={()=>setOpen(v=>!v)}><Plus size={14}/>{open?"Close":"Add schedule"}</button></div>{message&&<div className="panel"><b>{message}</b></div>}
