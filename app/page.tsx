...
export default function Home(){
 ...
 const addStream=()=>{if(!streamDraft.name.trim())return;const base=streamDraft.type==='Boolean'?'false':streamDraft.type==='String'?'ready':0;setStreams(s=>[...s,{id:Date.now(),name:streamDraft.name.trim(),deviceId:streamDraft.deviceId,type:streamDraft.type,unit:streamDraft.type==='Number'?streamDraft.unit:'',value:base}]);setStreamDraft({name:'',deviceId:devices[0]?.id||1,type:'Number',unit:'°C'});setShowStreamForm(false);setNotice('Datastream created')};
 const deleteStream=(id:number)=>{setStreams(s=>s.filter(x=>x.id!==id));setWidgets(w=>w.filter(x=>x.streamId!==id));setRules(r=>r.filter(x=>x.streamId!==id));setNotice('Datastream removed')};
 ...
 {tab==='Dashboard'&&<DashboardPanel widgets={widgets} streams={streams} onAdd={addWidget} onDelete={id=>setWidgets(w=>w.filter(x=>x.id!==id))} onUpdate={updateWidget}/>}
...
function DeviceDetailPanel({device,streams,onClose,onUpdateStream,setNotice}:{device:Device;streams:Stream[];onClose:()=>void;onUpdateStream:(id:number,value:string|number)=>void;setNotice:(v:string)=>void}){
 ...
 useEffect(()=>{void loadCommandHistory();const timer=setInterval(()=>void loadCommandHistory(),3000);return()=>clearInterval(timer)},[device.id,device.token]);
...
function DashboardPanel({widgets,streams,onAdd,onDelete,onUpdate}:{widgets:Widget[];streams:Stream[];onAdd:(k:Widget['kind'])=>void;onDelete:(id:number)=>void;onUpdate:(id:number,patch:Partial<Widget>)=>void}){
 ...
 return <div className="dashboardStudio">
  ...
  {widgets.length===0?<div className="dashboardEmpty"><div><b>Your dashboard is empty</b><span>Add a widget above to start building your control room.</span></div></div>:<div className="studioGrid">{widgets.map(w=>{const stream=streams.find(s=>s.id===w.streamId); return <article className="studioWidget" key={w.id}>
    ...
    <WidgetVisual widget={w} stream={stream}/>
   </article>})}</div>}
  {editing&&<div className="overlay" onMouseDown={()=>setEditing(null)}><div className="modal widgetEditor" onMouseDown={e=>e.stopPropagation()}><div className="modalHead"><div><span className="eyebrow">WIDGET CONFIGURATION</span><h2>{editing.title}</h2></div><button className="iconButton" onClick={()=>setEditing(null)}><X size={16}/></button></div><div className="form">
    ...
    <div className="editorPreview"><span>Live preview</span><WidgetVisual widget={editing} stream={currentStreams.find(s=>s.id===editing.streamId)}/></div>
...
function WidgetVisual({widget,stream}:{widget:Widget;stream?:Stream}){
 ...
 if(widget.kind==='Chart'){const base=Number(stream.value)||20;const points=Array.from({length:18},(_,i)=>{const v=base+(Math.sin(i*0.8)*2)+(i%3-1)*0.45;return Number(v.toFixed(1))});const min=Math.min(...points),max=Math.max(...points);const poly=points.map((v,i)=>`${(i/(points.length-1))*100},${100-((v-min)/Math.max(1,max-min))*82-9}`).join(' ');return <div className="chartWidget"><svg viewBox="0 0 100 100" preserveAspectRatio="none" aria-label="Telemetry trend"><polyline points={poly} fill="none" stroke="currentColor" strokeWidth="1.8" vectorEffect="non-scaling-stroke"/></svg><div className="chartMeta"><b>{stream.value}{stream.unit}</b><span>18-point simulated trend</span></div></div>}
 if(widget.kind==='Switch'){const on=String(stream.value)==='true';return <div className="widgetSwitch"><button className={on?'switchBig on':'switchBig'} onClick={()=>{}} aria-label="Simulated switch"><span/></button><div><b>{on?'ON':'OFF'}</b><small>{stream.name}</small></div></div>}
