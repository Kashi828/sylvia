...
export async function testPersistentAlertWebhook(id:string,ownerId:string){if(!databaseConfigured())return null;const r=await query("SELECT id,owner_id,project_id,name,device_id,stream_id,operator,threshold,severity,cooldown_seconds,enabled,action,webhook_url,last_triggered_at,active,created_at,updated_at FROM public.alert_rules WHERE id=$1 AND owner_id=$2 LIMIT 1",[id,ownerId]);if(!r.rows[0])return null;const rr=rule(r.rows[0] as Record<string,unknown>);const testId="test_"+Date.now();const evt:PersistentAlertEvent={id:testId,ownerId,projectId:rr.projectId,ruleId:rr.id,ruleName:rr.name,deviceId:rr.deviceId,streamId:rr.streamId,value:rr.threshold,threshold:rr.threshold,operator:rr.operator,severity:rr.severity,message:"Test alert for "+rr.name,timestamp:new Date().toISOString(),acknowledged:false};return {event:evt,delivery:await sendWebhook(rr,evt)};}