...
export async function POST(request:Request){
  ...
  try{return NextResponse.json({ok:true,member:await createWorkspaceInvitation({projectId:PROJECT,name,email,role})},{status:201});}catch(error){return NextResponse.json({ok:false,error:error instanceof Error?error.message:'Invitation failed'},{status:400});}
}
...
export async function PATCH(request:Request){
  ...
  try{const member=await updateWorkspaceMember(body.id,PROJECT,{name:body.name,role,status});if(!member)return NextResponse.json({ok:false,error:'Member not found'},{status:404});return NextResponse.json({ok:true,member});}catch(error){return NextResponse.json({ok:false,error:error instanceof Error?error.message:'Member update failed'},{status:400});}
}
...
export async function DELETE(request:Request){
  ...
  return NextResponse.json({ok:true,removed:await removeWorkspaceMember(id,PROJECT)});
}
