...
export async function POST(req:Request){
 ...
 const result=await testPersistentAlertWebhook(id,auth.ownerId,auth.projectId);return result?NextResponse.json({ok:true,...result,persistent:true}):NextResponse.json({ok:false,error:"Rule not found"},{status:404});
}