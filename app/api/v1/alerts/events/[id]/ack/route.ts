...
export async function POST(req:Request,{params}:{params:Promise<{id:string}>}){
 ...
 const event=await acknowledgePersistentAlertEvent(id,auth.ownerId);return event?NextResponse.json({ok:true,event,persistent:true}):NextResponse.json({ok:false,error:"Alert event not found"},{status:404});
}