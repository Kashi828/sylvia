...
export async function createPersistentCommand(id: string, deviceId: string | number, command: string, payload: unknown) {
  ...
  try {
    const result = await query("INSERT INTO sylvia_device_commands (id, device_id, command, payload, status) VALUES ($1,$2,$3,$4::jsonb,'queued') ON CONFLICT (id) DO NOTHING RETURNING id,device_id,command,payload,status,created_at,sent_at,acked_at,result", [id, String(deviceId), command, JSON.stringify(payload ?? null)]);
...
export async function markPersistentCommandSent(id: string) {
  if (!databaseConfigured()) return null;
  try {
    const result = await query("UPDATE sylvia_device_commands SET status='sent', sent_at=COALESCE(sent_at,now()) WHERE id=$1 RETURNING id,device_id,command,payload,status,created_at,sent_at,acked_at,result", [id]);
...
export async function getPersistentCommand(id: string) {
  if (!databaseConfigured()) return null;
  try {
    const result = await query("SELECT id,device_id,command,payload,status,created_at,sent_at,acked_at,result FROM sylvia_device_commands WHERE id=$1 LIMIT 1", [id]);
    const command = result.rows[0] ? normalize(result.rows[0] as Record<string, unknown>) : null;
...
export async function listPersistentPendingCommands(deviceId: string | number, limit = 10) {
  ...
  try {
    const result = await query("SELECT id,device_id,command,payload,status,created_at,sent_at,acked_at,result FROM sylvia_device_commands WHERE device_id=$1 AND status='queued' ORDER BY created_at ASC LIMIT $2", [String(deviceId), safeLimit(limit)]);
...
export async function recoverStalePersistentCommands(deviceId: string | number, timeoutSeconds = 120) {
  ...
  try {
    const result = await query(
      "UPDATE sylvia_device_commands SET status='failed', acked_at=now(), result=$3::jsonb WHERE device_id=$1 AND status='sent' AND sent_at IS NOT NULL AND sent_at < now() - ($2::text || ' seconds')::interval RETURNING id,device_id,command,payload,status,created_at,sent_at,acked_at,result",
...
export async function listPersistentCommands(deviceId: string | number, limit = 20) {
  if (!databaseConfigured()) return [];
  try {
    const result = await query("SELECT id,device_id,command,payload,status,created_at,sent_at,acked_at,result FROM sylvia_device_commands WHERE device_id=$1 ORDER BY created_at DESC LIMIT $2", [String(deviceId), safeLimit(limit)]);
    return result.rows.map(row => normalize(row as Record<string, unknown>));
  } catch { return []; }
}
...
export async function ackPersistentCommand(deviceId: string | number, id: string, resultValue: unknown) {
  ...
  try {
    ...
    const result = await query("UPDATE sylvia_device_commands SET status=$3, acked_at=now(), result=$4::jsonb WHERE id=$1 AND device_id=$2 AND status IN ('queued','sent') RETURNING id,device_id,command,payload,status,created_at,sent_at,acked_at,result", [id, String(deviceId), status, JSON.stringify(resultValue ?? null)]);
