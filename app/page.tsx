...
function ConnectivityPanel({devices,streams,apiKeys,setNotice}:{devices:Device[];streams:Stream[];apiKeys:ApiKey[];setNotice:(v:string)=>void}){
 ...
 const verifyStep=async(key:string,label:string,fn:()=>Promise<string>)=>{
   setVerification(v=>({...v,[key]:{status:'running',message:'Checking…'}}));
   try{const message=await fn();setVerification(v=>({...v,[key]:{status:'pass',message}}));}
   catch(error){setVerification(v=>({...v,[key]:{status:'fail',message:error instanceof Error?error.message:'Verification failed'}}));}
 };
 const runVerification=async()=>{
   if(!selected){setNotice('Select a registered device first');return;}
   await verifyStep('cloud','Cloud health',async()=>{const r=await fetch('/api/v1/health',{cache:'no-store'});const j=await r.json().catch(()=>null);if(!r.ok||j?.ready!==true)throw new Error(j?.error||'Cloud is not ready');return 'Database and MQTT readiness confirmed';});
   await verifyStep('device','Device identity',async()=>{const r=await fetch('/api/v1/devices/'+selected.id,{cache:'no-store'});const j=await r.json().catch(()=>null);if(!r.ok)throw new Error(j?.error||'Device lookup failed');return 'Owned device is reachable through the cloud API';});
   await verifyStep('streams','Datastreams',async()=>{const r=await fetch('/api/v1/datastreams?deviceId='+encodeURIComponent(String(selected.id)),{cache:'no-store'});const j=await r.json().catch(()=>null);if(!r.ok)throw new Error(j?.error||'Datastream lookup failed');const count=Array.isArray(j?.datastreams)?j.datastreams.length:0;if(!count)throw new Error('No persistent datastream configured');return count+' persistent datastream'+(count===1?'':'s')+' available';});
   await verifyStep('telemetry','Telemetry path',async()=>{const stream=streams.find(s=>s.deviceId===selected.id&&s.remoteId);if(!stream)throw new Error('No datastream ID available for telemetry verification');const r=await fetch('/api/v1/telemetry?deviceId='+selected.id+'&streamId='+encodeURIComponent(String(stream.remoteId)),{cache:'no-store'});const j=await r.json().catch(()=>null);if(!r.ok)throw new Error(j?.error||'Telemetry lookup failed');const samples=Array.isArray(j?.samples)?j.samples:[];const latest=samples.map((s:{timestamp?:string})=>Date.parse(String(s.timestamp||''))).filter(Number.isFinite).sort((a:number,b:number)=>b-a)[0];if(!samples.length)return 'Cloud telemetry endpoint reachable; waiting for first sample';if(latest&&Date.now()-latest>120000)return 'Persisted samples exist, but the latest sample is older than 2 minutes';return samples.length+' persisted samples found; latest sample is fresh';});
   await verifyStep('heartbeat','Hardware heartbeat',async()=>{const r=await fetch('/api/v1/devices/'+selected.id,{cache:'no-store'});const j=await r.json().catch(()=>null);if(!r.ok)throw new Error(j?.error||'Device state unavailable');if(!j?.device?.online)throw new Error('Device is offline — connect the ESP8266 and wait for heartbeat');const lastSeen=Date.parse(String(j?.device?.lastSeen||''));if(lastSeen&&Date.now()-lastSeen>45000)throw new Error('Device is marked online but heartbeat is older than 45 seconds');return 'Device is online and heartbeat is fresh';});
   await verifyStep('command','Command path',async()=>{const r=await fetch('/api/v1/devices/'+selected.id+'/command',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({command:'identify',payload:null})});const j=await r.json().catch(()=>null);if(!r.ok||j?.ok!==true)throw new Error(j?.error||'Command queue rejected the request');const commandId=String(j.commandId||'');if(!commandId)throw new Error('Command was accepted without a persistent command ID');const deadline=Date.now()+12000;while(Date.now()<deadline){await new Promise(resolve=>setTimeout(resolve,1500));const historyResponse=await fetch('/api/v1/devices/'+selected.id+'/commands?history=true&limit=10',{cache:'no-store'});const historyData=await historyResponse.json().catch(()=>null);const command=Array.isArray(historyData?.commands)?historyData.commands.find((item:{id?:string})=>String(item.id)===commandId):null;if(command?.status==='acked')return 'Identify command acknowledged by the device';if(command?.status==='failed')throw new Error('Identify command was acknowledged as failed');}return 'Command entered the persistent queue; device acknowledgement not received within 12 seconds';});
   setNotice('Hardware verification completed');
 };
 const copy=async(text:string)=>{try{await navigator.clipboard?.writeText(text);setNotice('Copied to clipboard')}catch{setNotice('Copy unavailable')}};
 const firmware=`/* SYLVIA v0.53.0-alpha.3 — official Arduino SDK starter */
