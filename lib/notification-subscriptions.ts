...
export async function listSubscriptions(userId='usr_owner',projectId='sylvia-local-workspace'){
 if(databaseConfigured()){try{const r=await query<any>('SELECT id,user_id,project_id,kind,severity,enabled,created_at,updated_at FROM notification_subscriptions WHERE user_id=$1 AND project_id=$2 ORDER BY created_at ASC',[userId,projectId]);if(r.rows.length)return r.rows.map(map)}catch{}}
...
export async function upsertSubscription(input:Partial<NotificationSubscription>&Pick<NotificationSubscription,'userId'|'projectId'>){
 const id=input.id||`sub_${Date.now()}_${Math.random().toString(36).slice(2,7)}`;const now=new Date().toISOString();const next:NotificationSubscription={id,userId:input.userId,projectId:input.projectId,kind:input.kind||'all',severity:input.severity||'all',enabled:input.enabled!==false,createdAt:now,updatedAt:now};
...
export async function deleteSubscription(id:string,userId="usr_owner",projectId="sylvia-local-workspace"){if(databaseConfigured()){try{await query('DELETE FROM notification_subscriptions WHERE id=$1 AND user_id=$2 AND project_id=$3',[id,userId,projectId]);return true}catch{}}const i=store.findIndex(s=>s.id===id&&s.userId===userId&&s.projectId===projectId);if(i>=0)store.splice(i,1);return true;}
